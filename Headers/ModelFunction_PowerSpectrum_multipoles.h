...
/**
 *  @file Headers/ModelFunction_PowerSpectrum_multipoles.h
 *
 *  @brief Global functions to model the multipoles of the 
 *  power spectrum
 *
 *  This file contains all the prototypes of the functions used
 *  to model the multipoles of the power spectrum
 */

#ifndef __MODFUNCPOWSPECMULTI__
#define __MODFUNCPOWSPECMULTI__
...
namespace cbl {
  ...
  namespace modelling {
    ...
    namespace powspec {
       ...
       *  analyses of the multipoles of the power spectrum
       */
      struct STR_data_model {
	...
	/// fiducial cosmology
	std::shared_ptr<cosmology::Cosmology> cosmology_fid;
	...
	std::vector<std::string> Cpar;
        ...
        /// vector of order of the multipoles
        std::vector<int> ll;
        
	/// vector of wave vector modules
	std::vector<double> kk;

	///  vector of fiducial bias value
	std::vector<double> bias;

        /// the shift tranverse to the l.o.s
        double q_perp;
        
        /// the shift parallel to the l.o.s
        double q_para;
	...
	STR_data_model () = default;
       ...
       *  \f$P(k, \mu)\f$ is computed by cbl::modelling::powspec::Pkmu
       *
       *  @param kk the wave vector module
       *
       ...
       *    cbl::modelling::powspec::Pkmu_dispersion
       *
       *  - the Scoccimarro model with either Gaussian or Lorentzian
       *    damping, implemented in
       *    cbl::modelling::powspec::Pkmu_Scoccimarro,
       *    cbl::modelling::powspec::Pkmu_Scoccimarro_fitPezzotta and
       *    cbl::modelling::powspec::Pkmu_Scoccimarro_fitBel
       *
       *  - the TNS model with either Gaussian or Lorentzian damping,
       *    implemented in cbl::modelling::powspec::Pkmu_TNS
       *
       *  - the extended model with either Gaussian or Lorentzian
       *    damping, implemented in cbl::modelling::powspec::Pkmu_eTNS
       *
       ...
       *  the Fingers of God effect at small scales and \f$A_{MC}\f$ is the
       *  mode coupling bias.
       *
       *  @param kk the wave vector module
       *
       ...
       *  which is computed by cbl::cosmology::PkXi::Pk_matter, and
       ...
       *  by cbl::cosmology::PkXiNonLinear::Pk_TNS_dd_dt_tt, \f$D_{FoG}\f$
       ...
       *  computed by cbl::cosmology::PkXi::Pk_matter, and
       ...
       *  Perturbation Theory as implemented in the CPT Library
       *  [http://www2.yukawa.kyoto-u.ac.jp/~atsushi.taruya/cpt_pack.html]
       *  by cbl::cosmology::PkXiNonLinear::Pk_TNS_dd_dt_tt, all the terms
       *  in the power spectrum correction correction terms are
       *  computed by
       *  cbl::cosmology::PkXiNonLinear::Pk_TNS_AB_terms_1loop,
       *  \f$D_{FoG}\f$ is a damping factor used to model the random
       *  peculiar motions at small scales, which can be either
       *  Gaussian:
       *
       ...
       *  by cbl::cosmology::PkXiNonLinear::Pk_TNS_dd_dt_tt, all the terms
       *  in the power spectrum correction correction terms are
       *  computed by cbl::cosmology::PkXiNonLinear::Pk_TNS_AB_terms_1loop
       *  and cbl::cosmology::PkXiNonLinear::Pk_eTNS_terms_1loop,
       *  \f$D_{FoG}\f$ is a damping factor used to model the random
       *  peculiar motions at small scales, which can be either
       *  Gaussian:
       *
      ...
      /**
       *  @brief the model for the multipole of the power spectrum
       *
       ...
       *
       *    - \f$ n \f$  cosmological parameters      
       *    
       *  the power spectrum is computed
       *  using the input cosmological parameters
       *
       *  @param kk the vector of wave number module at which the model is computed
       *
       *  @param inputs pointer to the structure that contains the multipoles of the power spectrum data model
       *
       *  @param parameter 1D vector containing the linear bias 
       *  
       *  @return the multipole of the power spectrum
       */
      std::vector<double> Pl_Kaiser (const std::vector<double> kk, const std::shared_ptr<void> inputs, std::vector<double> &parameter);
