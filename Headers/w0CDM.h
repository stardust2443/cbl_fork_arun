...
/**
 *  @file Headers/w0CDM.h
 *
 *  @brief The class w0CDM 
 *
 *  This file defines the interface of the class w0CDM, used for
 *  cosmological calculations in the flat w-cold dark matter
 *  model
 *
...
namespace cbl {
  ...
  namespace cosmology {
    ...
    /**
     *  @class w0CDM w0CDM.h "Headers/w0CDM.h"
     *
     *  @brief The class w0CDM
     *
     *  This class is used to handle objects of type <EM> w0CDM
     *  </EM>. They are used for cosmological calculations in the flat
     *  w-cold dark matter model
     */
    class w0CDM : public virtual LCDM {
    ...
    public:
      ...
      /**
       *  @brief Default constructor of the class w0CDM
       */
      w0CDM ()
	: LCDM() { m_model = "w0CDM"; }

      /**
       *  @brief Main Constructor of the class w0CDM
       *
       ...
       *  e.g. Mpc/h)
       */
      w0CDM (const double Omega_matter, const double Omega_baryon, const double Omega_radiation, const double little_h, const double scalar_amp, const double scalar_pivot, const double n_spec, const double tau, const double w0, const bool unit=true);

      /**
       *  @brief Constructor of the class w0CDM that uses built-in
       ...
       *  e.g. Mpc/h)
       */
      w0CDM (const std::string cosmological_model, const bool unit=true);

      /**
       *  @brief Default destructor
       */
      virtual ~w0CDM () = default;
      ...
      { return std::make_shared<w0CDM>(*this); }
      ...
      { return {"Omega_matter", "Omega_CDM", "Omega_baryon", "Omega_radiation", "little_h", "scalar_amp", "scalar_pivot", "sigma8", "n_spec", "tau", "w0"}; }
       ...
       *  DE}(z=0)} = \exp\left[3\int_0^z{\rm}
       *  dz'\frac{1+w(z')}{1+z'}\right] \f]
       *
       *  When \f$w(z) = w_0\f$, the integral above can be computed as
       *  follows (see e.g. Eq.(1.4) of Bassett & Hlozek 2010):
       *
       *  \f[ f_{\rm DE}(z) = (1+z)^{3(1+w_0)} \f]
       *
      ...
      /**
       *  @brief The density parameter
       *
       *  This member function computes the density parameter for a
       *  cosmic component with equation of state parameter w, at a
       *  given redshift (see e.g. Coles & Lucchin - Cosmology,
       *  Sec.2.3):
       *
       *  \f[ \Omega_w(z) = \Omega_w\frac{(1+z)^{3(1+w)}}{E^2(z)} \f]
       *
       *  where \f$\Omega_w\equiv\Omega_w(z=0)\f$ and \f$E(z)\f$ is
       *  computed by cosmology::Cosmology::EE
       *
       *  @param Omega_0w \f$\Omega_w\equiv\Omega_w(z=0)\f$: the
       *  density parameter at z=0
       *
       *  @param w \f$w=p/(\rho c^2)\f$: the equation of state
       *  parameter
       *
       *  @param redshift the redshift
       *
       *  @return \f$\Omega_w(z)\f$: the density parameter
       */
      double Omega_w (const double Omega_0w, const double w, const double redshift) const;
