...
/**
 *  @file Headers/kw0wanuCDM.h
 *
 *  @brief The class kw0wanuCDM 
 *
 *  This file defines the interface of the class kw0wanuCDM, used for
 *  cosmological calculations in the non-flat w0-wa-cold dark matter
...
#ifndef __kw0wanuCDM__
#define __kw0wanuCDM__
...
namespace cbl {
  ...
  namespace cosmology {
    ...
    /**
     *  @class kw0wanuCDM kw0wanuCDM.h "Headers/kw0wanuCDM.h"
     *
     *  @brief The class kw0wanuCDM
     *
     *  This class is used to handle objects of type <EM> kw0wanuCDM
     *  </EM>. They are used for cosmological calculations in the non-flat
     *  w0-wa-cold dark matter model with massive neutrinos
     */
    class kw0wanuCDM : public virtual w0wanuCDM, public virtual kLCDM {
    ...
    public:
      ...
      /**
       *  @brief Default constructor of the class kw0wanuCDM
       */
      kw0wanuCDM ()
	: w0wanuCDM(), kLCDM() { m_model = "kw0wanuCDM"; }

      /**
       *  @brief Main Constructor of the class kw0wanuCDM
       *
       ...
       *  to reionization
       *
       *  @param Omega_DE \f$\Omega_{\rm DE}\f$: the dark energy
       *  density at z=0
       *
       *  @param w0 \f$w_0\f$: the parameter of the dark energy
       *  equation of state (CPL parameterisation)
       *
       *  @param wa \f$w_a\f$: the parameter of the dark energy
       *  equation of state (CPL parameterisation)
       *
       ...
       *  e.g. Mpc/h)
       */
      kw0wanuCDM (const double Omega_matter, const double Omega_baryon, const double Omega_radiation, const double little_h, const double scalar_amp, const double scalar_pivot, const double n_spec, const double tau, const double Omega_DE, const double w0, const double wa, const double Omega_neutrinos, const double massless_neutrinos, const int massive_neutrinos, const bool unit=true);

      /**
       *  @brief Constructor of the class kw0wanuCDM that uses built-in
       ...
       *  e.g. Mpc/h)
       */
      kw0wanuCDM (const std::string cosmological_model, const bool unit=true);
      
      /**
       *  @brief Default destructor
       */
      virtual ~kw0wanuCDM () = default;
      ...
      { return std::make_shared<kw0wanuCDM>(*this); }
      ...
      { return {"Omega_matter", "Omega_CDM", "Omega_baryon", "Omega_radiation", "little_h", "scalar_amp", "scalar_pivot", "sigma8", "n_spec", "tau", "Omega_DE", "Omega_k", "w0", "wa", "Omega_neutrinos", "massless_neutrinos", "massive_neutrinos"}; }
       ...
       */
      virtual double f_DE (const double redshift) const override;
       ...
       *  \Omega_{\rm DE}f_{\rm DE}(z) + \Omega_K(1+z)^2 +
       *  \Omega_r(1+z)^4} \f]
       *
       *  where \f$f_{\rm DE}\f$ is computed by
       *  cbl::cosmology::Cosmology::f_DE
       *
       *  @param redshift the redshift 
       *
       ...
       *  
       *  \f[ \Omega_{rad}(z)+0.5\cdot \Omega_m(z)+(1+3(w_0+w_a\cdot (1-\frac{1}{1+z}))) \cdot \Omega_{DE}(z) \f]
       *
       *  @param redshift the redshift
       *  @return q
       */
       ...
       *
       *  The value is obtained by solving the following expression in term
       *  of the scale factor a
       *
       *  \f[ ae^{\frac{w_a(1-a)}{w_0+w_a}}= \left( \frac{-1}{1+3w_0+3w_a} \ \frac{\Omega_m}{\Omega_\Lambda} \right)^{\frac{-1}{3w_0+3w_a}}  \f]
       *
       *  Then the scale factor is converted in redshift using 
       *  \f[ a = \frac{1}{1+z} \f]
       *
       *  The lambert function [https://www.boost.org/doc/libs/develop/libs/math/doc/html/math_toolkit/lambert_w.html] is used to solve the transcendetal equation
       *
       ...
       *  of the scale factor a
       *
       *  \f[ ae^{\frac{w_a(1-a)}{w_0+w_a}}= \left( \frac{\Omega_m}{\Omega_\Lambda} \right)^{\frac{-1}{3w_0+3w_a}}  \f]   
       *
       *  Then the scale factor is converted in redshift using 
       *  \f[ a = \frac{1}{1+z} \f]
       *
       ...
       *  It is computed as 
       *
       *  \f[ \frac{c}{H_0} \int^z_0 \frac{dz}{E(z)} \f]
       *
       ...
       *
       *  where \f$ D(z) \f$ is the linear growth
       *  factor. Specifically, the linear growth rate is computed by
       *  solving the following differential equation (see e.g. Wu,
       *  Yu, and Fu 2009, Eq. 4):
       *
       *  \f[ \frac{d\,f(z)}{d\,\ln a(z)} + f^2(z) + \left(
       *  \frac{\dot{H}(z)}{H^2(z)}+2 \right)\,f(z) = \frac{3}{2}
       *  \Omega_m(z) \, . \f]
       *
       *  Specifically, in this cosmological model, the term \f$
       *  \dot{H}(z) \f$ can be written as follows:
       *
       *  \f[ \dot{H}(z) = -\left(1 + z\right) H(z) \frac{d H(z)}{d z}
       *  = -\left(1 + z\right) \left(\frac{\Omega_{DE} \left(-
       *  \frac{3 w_{a}}{\left(1 + z\right)^{2}} + \frac{3 w_{0} + 3
       *  w_{a} + 3}{1 + z}\right) e^{- 3 w_{a} + \frac{3 w_{a}}{1 +
       *  z} + 3 \left(w_{0} + w_{a} + 1\right) \log{\left(1 + z
       *  \right)}}}{2} + \frac{3 \Omega_{M} \left(1 +
       *  z\right)^{2}}{2} + \frac{\Omega_{k} \left(2 z + 2\right)}{2}
       *  + 2 \Omega_{r} \left(1 + z\right)^{3}\right) \f]
       *
