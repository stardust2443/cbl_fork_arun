...
CXX = g++
ifeq ($(SYS),Darwin)
  CXX := $(shell /opt/homebrew/bin/brew list gcc --versions | awk '{print "/opt/homebrew/bin/g++-" $$NF}' | sed 's/gcc@//; s/\..*//')
endif

# C compiler, used to compile CUBA libraries
CC = gcc
ifeq ($(SYS),Darwin)
	CC := $(shell /opt/homebrew/bin/brew list gcc --versions | awk '{print "/opt/homebrew/bin/gcc-" $$NF}' | sed 's/gcc@//; s/\..*//')
endif
...
# if on, compile and run the unit tests
Unit ?= off

# if on, allow the compiler to use optimization flags for CAMB
Opt ?= off

# if on, CAMB will be forced to recompile
forceCAMB ?= off

# if on, play the CBL music!
Music ?= off
Dir_music = $(PWD)/Music/
MP3 := $(Dir_music)cbl1.mp3 $(Dir_music)cbl2.mp3 $(Dir_music)cbl3.mp3
MP3_FILE = $(shell shuf -n 1 -e $(MP3))
...
dir_LIB_FFTW =
ifeq ($(SYS),Darwin)
	dir_INC_FFTW = /opt/homebrew/include
	dir_LIB_FFTW = /opt/homebrew/lib
endif
...
dir_LIB_cfitsio =
ifeq ($(SYS),Darwin)
	dir_INC_cfitsio = /opt/homebrew/include
	dir_LIB_cfitsio = /opt/homebrew/lib
endif

# boost installation directories
dir_INC_BOOST = $(shell $(PY) -c 'import sysconfig; print(sysconfig.get_config_var("INCLUDEDIR"))')
dir_LIB_BOOST =
...
Dir_HEALPix = External/HEALPix/Healpix_3.82
dir_HEALPix = $(addprefix $(PWD)/, $(Dir_HEALPix))
dir_hpx_extract = $(addprefix $(PWD)/, External/HEALPix)
...
HEALPix_LIB := $(dir_HEALPix)/lib
HEALPix_INC = $(dir_HEALPix)/include/healpix_cxx
HPX_CONF := $(shell if [ -e "$(dir_HEALPix)/bin/alice3" ]; then echo "$(dir_HEALPix)/bin/alice3"; fi)

ifeq ($(HEALPix_INC),)
  else
  FLAGS_INC := $(FLAGS_INC) -I$(HEALPix_INC)
endif

ifeq ($(HEALPix_LIB),)
else
  ifeq ($(SYS),Darwin)
    FLAGS_HEALPix = -Wl,-rpath,$(HEALPix_LIB) -L$(HEALPix_LIB) -L$(dir_LIB_cfitsio) -lhealpix_cxx -lsharp 
  else
    FLAGS_HEALPix = -Wl,-rpath,$(HEALPix_LIB) -L$(HEALPix_LIB) -lhealpix_cxx -lsharp -lcfitsio
  endif
endif
...
ifeq ($(dir_INC_cfitsio),)
    CCfits_COMPILE = cd $(dir_CCfits) && tar -xzf CCfits-2.6.tar.gz && cd CCfits && ./configure CXX=$(CXX) --prefix=$(dir_CCfits) CXXFLAGS="-w" && make && make install
else
    CCfits_COMPILE = cd $(dir_CCfits) && tar -xzf CCfits-2.6.tar.gz && cd CCfits && ./configure CXX=$(CXX) --with-cfitsio-include=$(dir_INC_cfitsio) --with-cfitsio-libdir=$(dir_LIB_cfitsio) --prefix=$(dir_CCfits) CXXFLAGS="-w" && make && make install
...
ifneq ($(dir_CUBA_root),)
    ...
    CUBA_COMPILE = cd $(dir_CUBA) && ./configure CC=$(CC) CFLAGS="-fPIC -std=gnu99" && make lib CC=$(CC)" -w" FC=$(F)" -w"
...
ifeq ($(python_version_major),2)
	...
	PYLIB = $(shell $(PY) -c 'import sysconfig; print sysconfig.get_config_var("LIBDIR")')
...
ifeq ($(python_version_major),3)
	PYINC = $(shell $(PY) -c 'import sysconfig; print(sysconfig.get_config_var("INCLUDEDIR"))')
	PYLIB = $(shell $(PY) -c 'import sysconfig; print(sysconfig.get_config_var("LIBDIR"))')
	ABIFLAGS = $(shell $(PY) -c 'import sysconfig; print(sysconfig.get_config_var("ABIFLAGS"))')
	SWIG_FLAG = -python -c++ -threads
...
Dir_MODEL_POWSPEC = Modelling/PowerSpectrum/
...
Dir_MODEL_SHEARPOW = Modelling/ShearShearLensing/
Dir_GLOB = GlobalFunc/
...
dir_MG = $(addprefix $(PWD)/,$(Dir_MG))
dir_FUNC = $(addprefix $(PWD)/,$(Dir_FUNC))
dir_EMU = $(addprefix $(PWD)/,$(Dir_EMU))
...
dir_MODEL_POWSPEC = $(addprefix $(PWD)/,$(Dir_MODEL_POWSPEC))
...
dir_MODEL_SHEARPOW = $(addprefix $(PWD)/,$(Dir_MODEL_SHEARPOW))
...
OBJ_COSM = $(dir_COSM)Cosmology.o $(dir_COSM)LCDM.o $(dir_COSM)kLCDM.o $(dir_COSM)w0CDM.o $(dir_COSM)kw0CDM.o $(dir_COSM)w0waCDM.o $(dir_COSM)kw0waCDM.o $(dir_COSM)LnuCDM.o $(dir_COSM)kLnuCDM.o $(dir_COSM)w0nuCDM.o $(dir_COSM)kw0nuCDM.o $(dir_COSM)w0wanuCDM.o $(dir_COSM)kw0wanuCDM.o $(dir_COSM)PkXi.o $(dir_COSM)Sigma.o $(dir_COSM)PkXizSpace.o $(dir_COSM)PkXiNonLinear.o $(dir_COSM)MassFunction.o $(dir_COSM)Bias.o $(dir_COSM)RSD.o $(dir_COSM)Velocities.o $(dir_COSM)MassGrowth.o $(dir_COSM)NG.o $(dir_COSM)BAO.o $(dir_COSM)SizeFunction.o  $(dir_COSM)3PCF.o $(OBJ_RECfast) $(dir_COSM)HaloProfile.o $(dir_COSM)SuperSampleCovariance.o
...
OBJ_CAT = $(dir_CAT)Object.o $(dir_CAT)Catalogue.o $(dir_CAT)RandomCatalogue.o $(dir_CAT)ChainMesh_Catalogue.o $(dir_CAT)RandomCatalogueVIPERS.o $(dir_CAT)VoidCatalogue.o $(dir_CAT)GadgetCatalogue.o $(dir_CAT)FITSCatalogue.o $(dir_CAT)HODCatalogue.o
...
OBJ_MAP = $(dir_MAP)MapCalculator.o
...
OBJ_MODEL_DENSITYPROFILE = $(dir_MODEL_DENSITYPROFILE)Modelling_DensityProfile.o $(dir_MODEL_DENSITYPROFILE)Modelling_DensityProfile_Auxiliary.o $(dir_MODEL_DENSITYPROFILE)Modelling_DensityProfile_Combined.o
...
OBJ_MODEL_POWSPEC = $(dir_MODEL_POWSPEC)Modelling_PowerSpectrum_multipoles.o $(dir_MODEL_POWSPEC)ModelFunction_PowerSpectrum_multipoles.o
...
OBJ_MODEL_SHEARPOW = $(dir_MODEL_SHEARPOW)Modelling_PowerSpectrum_Shear.o $(dir_MODEL_SHEARPOW)ModelFunction_PowerSpectrum_Shear.o 
...
OBJ_CBL = $(OBJ_PATH) $(OBJ_KERNEL) $(OBJ_WRAP_CAMB) $(OBJ_WRAP_LIB) $(OBJ_FUNCGRID) $(OBJ_FFT) $(OBJ_RAN) $(OBJ_FUNC) $(OBJ_EMU) $(OBJ_DATA) $(OBJ_FIELD) $(OBJ_HIST) $(OBJ_DISTR) $(OBJ_STAT) $(OBJ_COSM) $(OBJ_CM) $(OBJ_MG) $(OBJ_CAT) $(OBJ_LN) $(OBJ_NC) $(OBJ_STACKPROFILE) $(OBJ_TWOP) $(OBJ_THREEP) $(OBJ_ANGPOW) $(OBJ_MAP) $(OBJ_MODEL_GLOB) $(OBJ_MODEL_COSM) $(OBJ_MODEL_MASSOBSERVABLEREL) $(OBJ_MODEL_DENSITYPROFILE) $(OBJ_MODEL_NC) $(OBJ_MODEL_POWSPEC) $(OBJ_MODEL_TWOP) $(OBJ_MODEL_THREEP) $(OBJ_MODEL_ANGPOW) $(OBJ_MODEL_SHEARPOW) $(OBJ_GLOB) $(OBJ_PARF)
...
OBJ_PYTHON = $(OBJ_PATH) $(OBJ_KERNEL) $(OBJ_WRAP_CAMB) $(OBJ_WRAP_LIB) $(OBJ_FUNCGRID) $(OBJ_FFT) $(OBJ_RAN) $(OBJ_FUNC) $(OBJ_DATA) $(OBJ_FIELD) $(OBJ_HIST) $(OBJ_DISTR) $(OBJ_STAT) $(OBJ_COSM) $(OBJ_CM) $(OBJ_MG) $(OBJ_CAT) $(OBJ_LN) $(OBJ_NC) $(OBJ_STACKPROFILE) $(OBJ_TWOP) $(OBJ_THREEP) $(OBJ_MODEL_GLOB) $(OBJ_MODEL_COSM) $(OBJ_MODEL_MASSOBSERVABLEREL) $(OBJ_MODEL_DENSITYPROFILE) $(OBJ_MODEL_NC) $(OBJ_MODEL_POWSPEC) $(OBJ_MODEL_TWOP) $(OBJ_MODEL_THREEP) $(OBJ_MODEL_ANGPOW) $(OBJ_MODEL_SHEARPOW) $(OBJ_GLOB) $(OBJ_PARF)
...
ifeq ($(Music), on)
	@mpv --loop $(MP3_FILE) > /dev/null 2>&1 & echo $$! > .mp3pid && sleep 1
endif
	...
	$(MAKE) -j3 libCM
	$(call colorecho, "\n"Compiling the library: libMG... "\n")
	$(MAKE) -j3 libMG
	$(call colorecho, "\n"Compiling the library: libFUNC... "\n")
	$(MAKE) -j3 libFUNC
	$(call colorecho, "\n"Compiling the library: libEMU... "\n")
	$(MAKE) -j3 libEMU
	...
	$(MAKE) -j3 libMODEL_NC
	$(call colorecho, "\n"Compiling the library: libMODEL_POWSPEC... "\n")
	$(MAKE) -j3 libMODEL_POWSPEC
	...
	$(call colorecho, "\n"Compiling the library: libMODEL_SHEARPOW... "\n")
	$(MAKE) -j3 libMODEL_SHEARPOW
...
ifeq ($(Music), on)
	@if [ -f .mp3pid ]; then PID=$$(cat .mp3pid); kill -0 $$PID 2>/dev/null && kill $$PID && rm .mp3pid; fi
endif

	rm -rf Logo/logo && $(MAKE) logo
...
libMG: $(OBJ_MG) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libMG.$(ES) $(OBJ_MG) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM

libFUNC: $(OBJ_FUNC) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libFUNC.$(ES) $(OBJ_FUNC) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG

libEMU: $(OBJ_EMU) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libEMU.$(ES) $(OBJ_EMU) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC

libDATA: $(OBJ_DATA) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libDATA.$(ES) $(OBJ_DATA) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU

libFIELD: $(OBJ_FIELD) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libFIELD.$(ES) $(OBJ_FIELD) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA

libHIST: $(OBJ_HIST) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libHIST.$(ES) $(OBJ_HIST) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD

libDISTR: $(OBJ_DISTR) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libDISTR.$(ES) $(OBJ_DISTR) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST

libCOSM: $(OBJ_COSM) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libCOSM.$(ES) $(OBJ_COSM) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR

libSTAT: $(OBJ_STAT) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libSTAT.$(ES) $(OBJ_STAT) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM

libCAT: $(OBJ_CAT) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libCAT.$(ES) $(OBJ_CAT) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT

libLN: $(OBJ_LN) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libLN.$(ES) $(OBJ_LN) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT

libNC: $(OBJ_NC) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libNC.$(ES) $(OBJ_NC) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN

libSTACKPROFILE: $(OBJ_STACKPROFILE) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libSTACKPROFILE.$(ES) $(OBJ_STACKPROFILE) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC

libTWOP: $(OBJ_TWOP) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libTWOP.$(ES) $(OBJ_TWOP) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE

libTHREEP: $(OBJ_THREEP) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libTHREEP.$(ES) $(OBJ_THREEP) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP

libANGPOW: $(OBJ_ANGPOW) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libANGPOW.$(ES) $(OBJ_ANGPOW) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP

libMAP: $(OBJ_MAP) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libMAP.$(ES) $(OBJ_MAP) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) $(FLAGS_HEALPix) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP -lANGPOW

libMODEL_GLOB: $(OBJ_MODEL_GLOB) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libMODEL_GLOB.$(ES) $(OBJ_MODEL_GLOB) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCM -lMG -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP -lANGPOW -lMAP

libMODEL_COSM: $(OBJ_MODEL_COSM) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libMODEL_COSM.$(ES) $(OBJ_MODEL_COSM) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP -lANGPOW -lMAP -lMODEL_GLOB

libMODEL_DENSITYPROFILE: $(OBJ_MODEL_DENSITYPROFILE) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libMODEL_DENSITYPROFILE.$(ES) $(OBJ_MODEL_DENSITYPROFILE) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP -lANGPOW -lMAP -lMODEL_GLOB -lMODEL_COSM

libMODEL_MASSOBSERVABLEREL: $(OBJ_MODEL_MASSOBSERVABLEREL) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libMODEL_MASSOBSERVABLEREL.$(ES) $(OBJ_MODEL_MASSOBSERVABLEREL) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP -lANGPOW -lMAP -lMODEL_GLOB -lMODEL_COSM -lMODEL_DENSITYPROFILE

libMODEL_NC: $(OBJ_MODEL_NC) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libMODEL_NC.$(ES) $(OBJ_MODEL_NC) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP -lANGPOW -lMAP -lMODEL_GLOB -lMODEL_COSM -lMODEL_DENSITYPROFILE -lMODEL_MASSOBSERVABLEREL

libMODEL_POWSPEC: $(OBJ_MODEL_POWSPEC) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libMODEL_POWSPEC.$(ES) $(OBJ_MODEL_POWSPEC) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lFUNC -lEMU -lDATA -lFIELD -lCM -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP -lANGPOW  -lMAP -lMODEL_GLOB -lMODEL_COSM -lMODEL_DENSITYPROFILE -lMODEL_MASSOBSERVABLEREL -lMODEL_NC	

libMODEL_TWOP: $(OBJ_MODEL_TWOP) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libMODEL_TWOP.$(ES) $(OBJ_MODEL_TWOP) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP -lANGPOW -lMAP -lMODEL_GLOB -lMODEL_COSM -lMODEL_DENSITYPROFILE -lMODEL_MASSOBSERVABLEREL -lMODEL_NC -lMODEL_POWSPEC

libMODEL_THREEP: $(OBJ_MODEL_THREEP) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libMODEL_THREEP.$(ES) $(OBJ_MODEL_THREEP) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lFUNC -lEMU -lDATA -lFIELD -lCM -lMG -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP -lANGPOW -lMAP -lMODEL_GLOB -lMODEL_COSM -lMODEL_DENSITYPROFILE -lMODEL_MASSOBSERVABLEREL -lMODEL_NC -lMODEL_POWSPEC -lMODEL_TWOP

libMODEL_ANGPOW: $(OBJ_MODEL_ANGPOW) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libMODEL_ANGPOW.$(ES) $(OBJ_MODEL_ANGPOW) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lFUNC -lEMU -lDATA -lFIELD -lCM -lMG -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP -lANGPOW -lMAP -lMODEL_GLOB -lMODEL_COSM -lMODEL_DENSITYPROFILE -lMODEL_MASSOBSERVABLEREL -lMODEL_NC -lMODEL_POWSPEC -lMODEL_TWOP -lMODEL_THREEP
	
libMODEL_SHEARPOW: $(OBJ_MODEL_SHEARPOW) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libMODEL_SHEARPOW.$(ES) $(OBJ_MODEL_SHEARPOW) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lFUNC -lEMU -lDATA -lFIELD -lCM -lMG -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP -lANGPOW -lMAP -lMODEL_GLOB -lMODEL_COSM -lMODEL_DENSITYPROFILE -lMODEL_MASSOBSERVABLEREL -lMODEL_NC -lMODEL_POWSPEC -lMODEL_TWOP -lMODEL_THREEP -lMODEL_ANGPOW

libGLOB: $(OBJ_GLOB) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libGLOB.$(ES) $(OBJ_GLOB) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP -lANGPOW -lMAP -lMODEL_GLOB -lMODEL_COSM -lMODEL_DENSITYPROFILE -lMODEL_MASSOBSERVABLEREL -lMODEL_NC -lMODEL_POWSPEC -lMODEL_TWOP -lMODEL_THREEP -lMODEL_ANGPOW -lMODEL_SHEARPOW

libPARF: $(OBJ_PARF) $(PWD)/Makefile
	$(CXX) $(FLAGS_LINK) -o $(PWD)/libPARF.$(ES) $(OBJ_PARF) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -Wl,-rpath,$(PWD) -L$(PWD)/ -lPATH -lKERNEL -lWRAP_CAMB -lWRAP_LIB -lFUNCGRID -lFFT -lRAN -lCM -lMG -lFUNC -lEMU -lDATA -lFIELD -lHIST -lDISTR -lCOSM -lSTAT -lCAT -lLN -lNC -lSTACKPROFILE -lTWOP -lTHREEP -lANGPOW -lMAP -lMODEL_GLOB -lMODEL_COSM -lMODEL_DENSITYPROFILE -lMODEL_MASSOBSERVABLEREL -lMODEL_NC -lMODEL_POWSPEC -lMODEL_TWOP -lMODEL_THREEP -lMODEL_ANGPOW -lMODEL_SHEARPOW -lGLOB

libCBL: $(OBJ_CBL) $(PWD)/Makefile
	$(CXX) $(FLAGS_CAMB) $(FLAGS_LINK) $(OBJ_CBL) -o $(PWD)/libCBL.$(ES) -llapack -lblas $(CUBA_LIB) $(FLAGS_CCFITS) $(FLAGS_GSL) -lgomp $(FLAGS_FFTW) -lgfortran $(FLAGS_HEALPix)
...
ifeq ($(forceCAMB), on)
	$(call colorecho, "\nRemoving eventual CAMB executable and then compiing CAMB... \n") 	
	[ -e $(PWD)/External/CAMB/fortran/camb ] && rm -f $(PWD)/External/CAMB/fortran/camb
	cd $(PWD)/External/CAMB/forutils ; make clean F90C=$(F)
	cd $(PWD)/External/CAMB/fortran ; make clean F90C=$(F) COMPILER=$(F) && make F90C=$(F) COMPILER=$(F) FFLAGS="$(FLAGS_CAMB)"
endif
...
cfitsio_LIB = 
ifeq ($(SYS),Darwin)
	cfitsio_INC = $(dir_INC_cfitsio)
	cfitsio_LIB = $(dir_LIB_cfitsio)
else
	cfitsio_INC = /usr/include
	cfitsio_LIB = $(dir_CCfits)lib	
endif

HEALPix: 
	$(call colorecho, "\nCompiling the library: HEALPix...\n")
	@echo "4" >> $(dir_hpx_extract)/input.txt
	@echo $(CC) >> $(dir_hpx_extract)/input.txt
	@echo "-O3 -ffast-math -march=native" >> $(dir_hpx_extract)/input.txt
	@echo "1" >> $(dir_hpx_extract)/input.txt
	@echo $(CC) >> $(dir_hpx_extract)/input.txt
	@echo "-O3 -ffast-math -march=native" >> $(dir_hpx_extract)/input.txt
	@echo $(CXX) >> $(dir_hpx_extract)/input.txt
	@echo "-O3" >> $(dir_hpx_extract)/input.txt
	@echo "1" >> $(dir_hpx_extract)/input.txt
	@echo $(cfitsio_INC) >> $(dir_hpx_extract)/input.txt
	@echo $(cfitsio_LIB) >> $(dir_hpx_extract)/input.txt

ifneq ($(strip $(HPX_CONF)),)
	@echo "\n\n\033[0;34mHEALPix already installed...skipping re-installation\033[0m \n\n"
endif

ifeq ($(strip $(HPX_CONF)),)
	cd $(dir_hpx_extract) && tar -xzf Healpix_3.82_2022Jul28.tar.gz
	cd $(dir_HEALPix) && cat ../input.txt | ./configure -L > ../healpix_config.log
	cd $(dir_HEALPix) && make -j3 cpp-all
	@echo "\n\n \033[0;32mHEALPix successfully installed\033[0m \n\n"
	rm $(dir_hpx_extract)/input.txt
endif
...
test:
	...
	cd $(PWD)/UnitTests/Cosmology ; make Cosmologies_test CXX=$(CXX) ; ./Cosmologies_test --log_level=test_suite > Cosmologies_log.txt
	cd $(PWD)/UnitTests/Cosmology ; make Sigma_test CXX=$(CXX) ; ./Sigma_test --log_level=test_suite > Sigma_log.txt
	cd $(PWD)/UnitTests/Cosmology ; make w0wanuCDM_test CXX=$(CXX) ; ./w0wanuCDM_test --log_level=test_suite > w0wanuCDM_log.txt
	cd $(PWD)/UnitTests/MapCalculator ; make Maps_test CXX=$(CXX) ; ./Maps_test --log_level=test_suite > Maps_log.txt
...
allExamples:
	...
	$(call colorecho, "\n"Compiling the example code: xi_CAMB_emulator.cpp ... "\n")
	cd $(PWD)/Examples/emulators/ ; make CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: LCDM.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make LCDM CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	...
	$(call colorecho, "\n"Compiling the example code: growth_rate.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make growth_rate CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	...
	$(call colorecho, "\n"Compiling the example code: velocities.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make velocities CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: three_point_correlation.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make three_point_correlation CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: size_function.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make size_function CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: ng.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make ng CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: bias.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make bias CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: mass_function.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make mass_function CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: fsigma8.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make fsigma8 CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: bao.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make bao CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: mass_growth.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make mass_growth CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: Pk_DeltaDelta.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make Pk_DeltaDelta CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: xi0_Kaiser.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make xi0_Kaiser CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: sigma.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make sigma CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: xi_matter.cpp ... "\n")
	cd $(PWD)/Examples/cosmology ; make xi_matter CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	...
	$(call colorecho, "\n"Compiling the example code: meshGridTest.cpp ... "\n")
	cd $(PWD)/Examples/meshGrid/codes ; make meshGridTest CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	...
	cd $(PWD)/Examples/clustering/codes ; make power_spectrum_angular CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: model_power_spectrum_angular.cpp ... "\n")
	cd $(PWD)/Examples/clustering/codes ; make model_power_spectrum_angular CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	$(call colorecho, "\n"Compiling the example code: model_power_spectrum_multipoles.cpp ... "\n")
	cd $(PWD)/Examples/clustering/codes ; make model_power_spectrum_multipoles CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
	...
	$(call colorecho, "\n"Compiling the example code: maps.cpp ... "\n")
	cd $(PWD)/Examples/maps/ ; make maps CXX=$(CXX) FLAGS_INC='$(FLAGS_INC)'
...
python:	
	...
	make pythonModelling_PowerSpectrum 
	...
	make pythonModelling_PowerSpectrumShear
	make pythonGlobalFunc
	make pythonMapCalculator
...
cleanTests:
	cd $(PWD)/UnitTests/Cosmology ; make clean
	cd $(PWD)/UnitTests/MapCalculator ; make clean

cleanExamples:
	cd $(PWD)/Examples/vectors ; make clean
	cd $(PWD)/Examples/eigen ; make clean 
	cd $(PWD)/Examples/randomNumbers ; make clean 
	cd $(PWD)/Examples/histogram ; make clean 
	cd $(PWD)/Examples/wrappers ; make clean 
	cd $(PWD)/Examples/covsample ; make clean 
	cd $(PWD)/Examples/emulators ; make clean 	
	cd $(PWD)/Examples/cosmology ; make clean 
	cd $(PWD)/Examples/data ; make clean 
	cd $(PWD)/Examples/statistics/codes ; make clean
	cd $(PWD)/Examples/meshGrid/codes ; make clean
	cd $(PWD)/Examples/lognormal/codes ; make clean 
	cd $(PWD)/Examples/catalogue ; make clean 
	cd $(PWD)/Examples/HOD/codes ; make clean 
	cd $(PWD)/Examples/numberCounts/codes ; make clean 
	cd $(PWD)/Examples/clustering/codes ; make clean 
	cd $(PWD)/Examples/cosmicVoids/codes ; make clean 
	cd $(PWD)/Examples/maps ; make clean 
	cd $(PWD)/Examples/parameterFile ; make clean 

	rm -rf $(PWD)/Examples/cosmology/results* $(PWD)/Examples/histogram/*.dat $(PWD)/Examples/statistics/output/* $(PWD)/Examples/lognormal/output/* $(PWD)/Examples/HOD/output/* $(PWD)/Examples/HOD/codes/.ipynb* $(PWD)/Examples/numberCounts/output/* $(PWD)/Examples/clustering/output/* $(PWD)/Examples/clustering/input/ran* $(PWD)/Examples/clustering/codes/.ipynb* $(PWD)/Examples/cosmicVoids/output/*
...
cleanTEMP:
	rm -f $(OBJ_ALL) core* $(PWD)/*~ $(dir_PATH)*~ $(dir_KERNEL)*~ $(dir_WRAP_CAMB)*~ $(dir_WRAP_LIB)*~ $(dir_FUNCGRID)*~ $(dir_FFT)*~ $(dir_RAN)*~ $(dir_CM)*~ $(dir_MG)*~ $(dir_FUNC)*~ $(dir_EMU)*~ $(dir_DATA)*~ $(dir_FIELD)*~ $(dir_HIST)*~ $(dir_DISTR)*~ $(dir_STAT)*~ $(dir_COSM)*~ $(dir_CM)*~ $(dir_CAT)*~ $(dir_LN)*~ $(dir_NC)*~ $(dir_STACKPROFILE)*~ $(dir_TWOP)*~ $(dir_THREEP)*~ $(dir_ANGPOW)*~ $(dir_MODEL_GLOB)*~ $(dir_MODEL_COSM)*~ $(dir_MODEL_MASSOBSERVABLEREL)*~ $(dir_MODEL_DENSITYPROFILE)*~ $(dir_MODEL_NC)*~ $(dir_MODEL_POWSPEC)*~ $(dir_MODEL_TWOP)*~ $(dir_MODEL_THREEP)*~ $(dir_MODEL_ANGPOW)*~ $(dir_MODEL_SHEARPOW)*~ $(Dir_MAP)*~ $(dir_GLOB)*~ $(dir_PARF)*~ $(dir_H)*~ $(PWD)/\#* $(dir_PATH)\#* $(dir_KERNEL)\#* $(dir_WRAP_CAMB)\#* $(dir_WRAP_LIB)\#* $(dir_FUNCGRID)\#* $(dir_FFT)\#* $(dir_RAN)\#* $(dir_CM)\#* $(dir_MG)\#* $(dir_FUNC)\#* $(dir_EMU)\#* $(dir_DATA)\#* $(dir_FIELD)\#* $(dir_HIST)\#*  $(dir_DISTR)\#* $(dir_STAT)\#* $(dir_COSM)\#* $(dir_CM)\#* $(dir_CAT)\#* $(dir_LN)\#* $(dir_TWOP)\#* $(dir_THREEP)\#* $(dir_MODEL_GLOB)\#* $(dir_MODEL_COSM)\#* $(dir_MODEL_MASSOBSERVABLEREL)\#* $(dir_MODEL_DENSITYPROFILE)\#* $(dir_MODEL_MASSOBSERVABLEREL)\#* $(dir_MODEL_DENSITYPROFILE)\#* $(dir_MODEL_NC)\#* $(dir_MODEL_POWSPEC)\#*  $(dir_MODEL_TWOP)\#* $(dir_MODEL_THREEP)\#* $(dir_MODEL_ANGPOW)\#* $(dir_MODEL_SHEARPOW)\#* $(dir_GLOB)\#* $(dir_PARF)\#* $(dir_H)\#* $(PWD)/Doc/WARNING_LOGFILE* $(PWD)/Doc/*~ $(PWD)/.mp3pid
...
purgeALL:
	...
	rm -rf Logo/logo
	rm -f Wrappers/CAMBwrapper/*.d Wrappers/CAMBwrapper/*.mod
	rm -f UnitTests_log.txt
	cd External/HEALPix/ && find . -mindepth 1 -maxdepth 1 ! -name "Healpix_3.82_2022Jul28.tar.gz" -exec rm -rf {} +;
...
$(dir_EMU)Emulator.o: $(dir_EMU)Emulator.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) $(Dvar) -c -fPIC $(FLAGS_INC) $(dir_EMU)Emulator.cpp -o $(dir_EMU)Emulator.o
...
$(dir_MG)MeshGrid.o: $(dir_MG)MeshGrid.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_MG)MeshGrid.cpp -o $(dir_MG)MeshGrid.o
...
$(dir_COSM)LCDM.o: $(dir_COSM)LCDM.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_COSM)LCDM.cpp -o $(dir_COSM)LCDM.o

$(dir_COSM)kLCDM.o: $(dir_COSM)kLCDM.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_COSM)kLCDM.cpp -o $(dir_COSM)kLCDM.o

$(dir_COSM)w0CDM.o: $(dir_COSM)w0CDM.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_COSM)w0CDM.cpp -o $(dir_COSM)w0CDM.o

$(dir_COSM)kw0CDM.o: $(dir_COSM)kw0CDM.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_COSM)kw0CDM.cpp -o $(dir_COSM)kw0CDM.o

$(dir_COSM)w0waCDM.o: $(dir_COSM)w0waCDM.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_COSM)w0waCDM.cpp -o $(dir_COSM)w0waCDM.o

$(dir_COSM)kw0waCDM.o: $(dir_COSM)kw0waCDM.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_COSM)kw0waCDM.cpp -o $(dir_COSM)kw0waCDM.o

$(dir_COSM)LnuCDM.o: $(dir_COSM)LnuCDM.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_COSM)LnuCDM.cpp -o $(dir_COSM)LnuCDM.o

$(dir_COSM)kLnuCDM.o: $(dir_COSM)kLnuCDM.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_COSM)kLnuCDM.cpp -o $(dir_COSM)kLnuCDM.o

$(dir_COSM)w0nuCDM.o: $(dir_COSM)w0nuCDM.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_COSM)w0nuCDM.cpp -o $(dir_COSM)w0nuCDM.o

$(dir_COSM)kw0nuCDM.o: $(dir_COSM)kw0nuCDM.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_COSM)kw0nuCDM.cpp -o $(dir_COSM)kw0nuCDM.o

$(dir_COSM)w0wanuCDM.o: $(dir_COSM)w0wanuCDM.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_COSM)w0wanuCDM.cpp -o $(dir_COSM)w0wanuCDM.o

$(dir_COSM)kw0wanuCDM.o: $(dir_COSM)kw0wanuCDM.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_COSM)kw0wanuCDM.cpp -o $(dir_COSM)kw0wanuCDM.o
...
$(dir_MAP)MapCalculator.o: $(dir_MAP)MapCalculator.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_MAP)MapCalculator.cpp -o $(dir_MAP)MapCalculator.o
...
$(dir_MODEL_DENSITYPROFILE)Modelling_DensityProfile_Auxiliary.o: $(dir_MODEL_DENSITYPROFILE)Modelling_DensityProfile_Auxiliary.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_MODEL_DENSITYPROFILE)Modelling_DensityProfile_Auxiliary.cpp -o $(dir_MODEL_DENSITYPROFILE)Modelling_DensityProfile_Auxiliary.o

$(dir_MODEL_DENSITYPROFILE)Modelling_DensityProfile_Combined.o: $(dir_MODEL_DENSITYPROFILE)Modelling_DensityProfile_Combined.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_MODEL_DENSITYPROFILE)Modelling_DensityProfile_Combined.cpp -o $(dir_MODEL_DENSITYPROFILE)Modelling_DensityProfile_Combined.o
...
$(dir_MODEL_POWSPEC)Modelling_PowerSpectrum_multipoles.o: $(dir_MODEL_POWSPEC)Modelling_PowerSpectrum_multipoles.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_MODEL_POWSPEC)Modelling_PowerSpectrum_multipoles.cpp -o $(dir_MODEL_POWSPEC)Modelling_PowerSpectrum_multipoles.o

$(dir_MODEL_POWSPEC)ModelFunction_PowerSpectrum_multipoles.o: $(dir_MODEL_POWSPEC)ModelFunction_PowerSpectrum_multipoles.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_MODEL_POWSPEC)ModelFunction_PowerSpectrum_multipoles.cpp -o $(dir_MODEL_POWSPEC)ModelFunction_PowerSpectrum_multipoles.o
...
$(dir_MODEL_SHEARPOW)Modelling_PowerSpectrum_Shear.o: $(dir_MODEL_SHEARPOW)Modelling_PowerSpectrum_Shear.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_MODEL_SHEARPOW)Modelling_PowerSpectrum_Shear.cpp -o $(dir_MODEL_SHEARPOW)Modelling_PowerSpectrum_Shear.o

$(dir_MODEL_SHEARPOW)ModelFunction_PowerSpectrum_Shear.o: $(dir_MODEL_SHEARPOW)ModelFunction_PowerSpectrum_Shear.cpp $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) -c -fPIC $(FLAGS_INC) $(dir_MODEL_SHEARPOW)ModelFunction_PowerSpectrum_Shear.cpp -o $(dir_MODEL_SHEARPOW)ModelFunction_PowerSpectrum_Shear.o
...
pythonEmulator: $(dir_Python)/Build $(dir_Python)Build/Emulator_wrap.o $(dir_Python)Lib/Emulator.i
	$(call insertLine, "from cblEmulator import *", $(dir_Python)CosmoBolognaLib/__init__.py)

$(dir_Python)Build/Emulator_wrap.cxx: $(dir_Python)Lib/Emulator.i $(HH) $(PWD)/Makefile
	$(call colorecho, "\n"Wrapping Emulator with swig... "\n")	
	$(SWIG) $(SWIG_FLAG) -I$(dir_H) -outdir $(dir_Python) -o $(dir_Python)Build/Emulator_wrap.cxx $(dir_Python)Lib/Emulator.i

$(dir_Python)Build/Emulator_wrap.o: $(dir_Python)Build/Emulator_wrap.cxx $(dir_Python)Lib/Emulator.i $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) $(SWIG_FLAG_ADD) $(PFLAGS) -c -fPIC $(FLAGS_INC) $(dir_Python)Build/Emulator_wrap.cxx -o $(dir_Python)Build/Emulator_wrap.o
	$(CXX) $(FLAGS_LINK) -o $(dir_Python)_cblEmulator.so $(dir_Python)Build/Emulator_wrap.o -Wl,-rpath,$(PWD)/ -L$(PWD)/ -lCBL
...
pythonMeshGrid: $(dir_Python)/Build $(dir_Python)Build/MeshGrid_wrap.o $(dir_Python)Lib/MeshGrid.i
	$(call insertLine, "from cblMeshGrid import *", $(dir_Python)CosmoBolognaLib/__init__.py)

$(dir_Python)Build/MeshGrid_wrap.cxx: $(dir_Python)Lib/MeshGrid.i $(HH) $(PWD)/Makefile
	$(call colorecho, "\n"Wrapping MeshGrid with swig... "\n")	
	$(SWIG) $(SWIG_FLAG) -I$(dir_H) -outdir $(dir_Python) -o $(dir_Python)Build/MeshGrid_wrap.cxx $(dir_Python)Lib/MeshGrid.i

$(dir_Python)Build/MeshGrid_wrap.o: $(dir_Python)Build/MeshGrid_wrap.cxx $(dir_Python)Lib/MeshGrid.i $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) $(SWIG_FLAG_ADD) $(PFLAGS) -c -fPIC $(FLAGS_INC) $(dir_Python)Build/MeshGrid_wrap.cxx -o $(dir_Python)Build/MeshGrid_wrap.o
	$(CXX) $(FLAGS_LINK) -o $(dir_Python)_cblMeshGrid.so $(dir_Python)Build/MeshGrid_wrap.o -Wl,-rpath,$(PWD)/ -L$(PWD)/ -lCBL
...
pythonModelling_PowerSpectrum: $(dir_Python)/Build $(dir_Python)Build/Modelling_PowerSpectrum_wrap.o $(dir_Python)Lib/Modelling_PowerSpectrum.i
	$(call insertLine, "from cblModelling_PowerSpectrum import *", $(dir_Python)CosmoBolognaLib/__init__.py)

$(dir_Python)Build/Modelling_PowerSpectrum_wrap.cxx: $(dir_Python)Lib/Modelling_PowerSpectrum.i $(HH) $(PWD)/Makefile
	$(call colorecho, "\n"Wrapping Modelling_PowerSpectrum with swig... "\n")	
	$(SWIG) $(SWIG_FLAG) -I$(dir_H) -outdir $(dir_Python) -o $(dir_Python)Build/Modelling_PowerSpectrum_wrap.cxx $(dir_Python)Lib/Modelling_PowerSpectrum.i

$(dir_Python)Build/Modelling_PowerSpectrum_wrap.o: $(dir_Python)Build/Modelling_PowerSpectrum_wrap.cxx $(dir_Python)Lib/Modelling_PowerSpectrum.i $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) $(SWIG_FLAG_ADD) $(PFLAGS) -c -fPIC $(FLAGS_INC) $(dir_Python)Build/Modelling_PowerSpectrum_wrap.cxx -o $(dir_Python)Build/Modelling_PowerSpectrum_wrap.o
	$(CXX) $(FLAGS_LINK) -o $(dir_Python)_cblModelling_PowerSpectrum.so $(dir_Python)Build/Modelling_PowerSpectrum_wrap.o -Wl,-rpath,$(PWD)/ -L$(PWD)/ -lCBL
...
pythonModelling_PowerSpectrumShear: $(dir_Python)/Build $(dir_Python)Build/Modelling_PowerSpectrumShear_wrap.o $(dir_Python)Lib/Modelling_PowerSpectrumShear.i
	$(call insertLine, "from cblModelling_PowerSpectrumShear import *", $(dir_Python)CosmoBolognaLib/__init__.py)

$(dir_Python)Build/Modelling_PowerSpectrumShear_wrap.cxx: $(dir_Python)Lib/Modelling_PowerSpectrumShear.i $(HH) $(PWD)/Makefile
	$(call colorecho, "\n"Wrapping Modelling_PowerSpectrumShear with swig... "\n")	
	$(SWIG) $(SWIG_FLAG) -I$(dir_H) -outdir $(dir_Python) -o $(dir_Python)Build/Modelling_PowerSpectrumShear_wrap.cxx $(dir_Python)Lib/Modelling_PowerSpectrumShear.i

$(dir_Python)Build/Modelling_PowerSpectrumShear_wrap.o: $(dir_Python)Build/Modelling_PowerSpectrumShear_wrap.cxx $(dir_Python)Lib/Modelling_PowerSpectrumShear.i $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) $(SWIG_FLAG_ADD) $(PFLAGS) -c -fPIC $(FLAGS_INC) $(dir_Python)Build/Modelling_PowerSpectrumShear_wrap.cxx -o $(dir_Python)Build/Modelling_PowerSpectrumShear_wrap.o
	$(CXX) $(FLAGS_LINK) -o $(dir_Python)_cblModelling_PowerSpectrumShear.so $(dir_Python)Build/Modelling_PowerSpectrumShear_wrap.o -Wl,-rpath,$(PWD)/ -L$(PWD)/ -lCBL
...
pythonMapCalculator: $(dir_Python)/Build $(dir_Python)Build/MapCalculator_wrap.o $(dir_Python)Lib/MapCalculator.i
	$(call insertLine, "from cblMapCalculator import *", $(dir_Python)CosmoBolognaLib/__init__.py)

$(dir_Python)Build/MapCalculator_wrap.cxx: $(dir_Python)Lib/MapCalculator.i $(HH) $(PWD)/Makefile
	$(call colorecho, "\n"Wrapping MapCalculator with swig... "\n")	
	$(SWIG) $(SWIG_FLAG) -I$(dir_H) -outdir $(dir_Python) -o $(dir_Python)Build/MapCalculator_wrap.cxx $(dir_Python)Lib/MapCalculator.i

$(dir_Python)Build/MapCalculator_wrap.o: $(dir_Python)Build/MapCalculator_wrap.cxx $(dir_Python)Lib/MapCalculator.i $(HH) $(PWD)/Makefile
	$(CXX) $(FLAGST) $(SWIG_FLAG_ADD) $(PFLAGS) -c -fPIC $(FLAGS_INC) $(dir_Python)Build/MapCalculator_wrap.cxx -o $(dir_Python)Build/MapCalculator_wrap.o
	$(CXX) $(FLAGS_LINK) -o $(dir_Python)_cblMapCalculator.so $(dir_Python)Build/MapCalculator_wrap.o -Wl,-rpath,$(PWD)/ -L$(PWD)/ -lCBL
###
...
$(PWD)/External/CAMB/fortran/camb:
	cd $(PWD)/External/CAMB/forutils ; make clean F90C=$(F)
	cd $(PWD)/External/CAMB/fortran ; make clean F90C=$(F) COMPILER=$(F) && make F90C=$(F) COMPILER=$(F) FFLAGS="$(FLAGS_CAMB) -w" && make clean F90C=$(F) COMPILER=$(F)

$(PWD)/External/CLASS/class:
	cd $(PWD)/External/CLASS ; mkdir -p output && make clean && make CC=$(CC) PYTHON=$(PY) OPTFLAG="-O3 -w" && make clean

$(PWD)/External/MPTbreeze-v1/mptbreeze:
	cd External/MPTbreeze-v1/Cuba-1.4/ ; ./configure CC=$(CC)" -w" F77=$(F)" -w" && make lib
	cd $(PWD)/External/MPTbreeze-v1 ; ./compile.sh $(F)

$(PWD)/External/mangle/bin/ransack:
	cd $(PWD)/External/mangle/src && mkdir -p ../bin && chmod +x configure && ./configure && make CC=$(CC)" -w" F77=$(F)" -w" && make clean
...
$(PWD)/External/CPT_Library/read_pk_sum:
	cd $(PWD)/External/CPT_Library ; make F90C=$(F) switch="-O3 -w" -Wargument-mismatch

$(PWD)/External/CAMB_SPT_private/camb:
	cd $(PWD)/External/CAMB_SPT_private ; make clean && make F90C=$(F) && make clean

$(PWD)/External/MGCAMB/camb:
	cd $(PWD)/External/MGCAMB ; make clean F90C=$(F) COMPILER=$(F) && make F90C=$(F) COMPILER=$(F)" -w" && make clean F90C=$(F) COMPILER=$(F)
