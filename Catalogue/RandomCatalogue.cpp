...
cbl::catalogue::Catalogue::Catalogue (const RandomType type, const std::shared_ptr<cosmology::Cosmology> real_cosmology, const std::shared_ptr<cosmology::Cosmology> test_cosmology, const std::string dir_in, const double Zguess_min, const double Zguess_max)
{
  ...
  for (size_t i=0; i<random_X.size(); i++) random_redshift[i] = real_cosmology->Redshift(random_dc[i], Zguess_min, Zguess_max);
  for (size_t i=0; i<random_X.size(); i++) random_dc_warped[i] = test_cosmology->D_C(random_redshift[i]);
...
cbl::catalogue::Catalogue::Catalogue (const RandomType type, const Catalogue catalogue, const double N_R, const int nbin, const std::shared_ptr<cosmology::Cosmology> cosmology, const bool conv, const double sigma, const std::vector<double> redshift, const std::vector<double> RA, const std::vector<double> Dec, const int z_ndigits, const int seed)
{ 
  ...
  if (type==RandomType::_createRandom_box_) {
    ...
    if (cosmology==nullptr) ErrorCBL("the input cosmology must be provided!", "Catalogue", "RandomCatalogue.cpp");
	...
	random::UniformRandomNumbers ran(0., catalogue.nObjects()-1, seed * i);
	observedCoordinates coord = {ra[i], dec[i], round_to_precision(catalogue.redshift(ran()), z_ndigits)};
	m_object.push_back(move(Object::Create(ObjectType::_Random_, coord, cosmology)));
      }
    }
...
cbl::catalogue::Catalogue::Catalogue (const RandomType type, const Catalogue catalogue, const double N_R, const int nbin, const double Angle, const std::vector<double> redshift, const std::shared_ptr<cosmology::Cosmology> cosmology, const bool conv, const double sigma, const int seed)
{
  ...
  for (auto &&red : random_redshift) 
    DD.emplace_back(cosmology->D_C(red));

  // constructing the random sample
  
  const double rMax = cosmology->D_C(catalogue.Max(Var::_Redshift_))*tan(Angle);
...
cbl::catalogue::Catalogue::Catalogue (const RandomType type, const std::vector<std::string> mangle_mask, const Catalogue catalogue, const double N_R, const int nbin, const std::shared_ptr<cosmology::Cosmology> cosmology, const bool conv, const double sigma, const int seed)
{
...
cbl::catalogue::Catalogue::Catalogue (const RandomType type, const Catalogue catalogue, const double N_R, const bool dndz_per_stripe, const int nbin, const std::shared_ptr<cosmology::Cosmology> cosmology, const bool conv, const double sigma, const int seed)
{
...
cbl::catalogue::Catalogue::Catalogue (const RandomType type, Catalogue catalogue, const double N_R, const std::shared_ptr<cosmology::Cosmology> cosmology, const std::vector<double> RA_range, const std::vector<double> DEC_range, const unsigned int nbin, const int seed)
{
  ...
  auto prop = catalogue.compute_catalogueProperties_lightCone(cosmology, RA_range, DEC_range, nbin);
...
cbl::catalogue::Catalogue::Catalogue (const RandomType type, std::shared_ptr<Catalogue> catalogue, const int seed)
{
  if (type==RandomType::_createRandom_box_) {
  ...
  else ErrorCBL("the random catalogue has to be of type _createRandom_box_!", "Catalogue", "RandomCatalogue.cpp");
}
...
void cbl::catalogue::Catalogue::equalize_random_lightCone (cbl::catalogue::Catalogue tracer_catalogue, const std::shared_ptr<cosmology::Cosmology> cosmology, const std::vector<double> RA_range, const std::vector<double> DEC_range, const int seed) 
{
  if (nObjects()>tracer_catalogue.nObjects()) {
    ...
    auto prop = tracer_catalogue.compute_catalogueProperties_lightCone(cosmology, RA_range, DEC_range, 100);
    ...
    for (int i=0; i<int(tracer_catalogue.nObjects()-nObjec); i++) {
      observedCoordinates coord = {randomRA(), asin(randomDec()), interpolated(distribution(generator), nObj, zbins, "Linear")};
      add_object(Object::Create(ObjectType::_Random_, coord, cosmology));
    }
  }
}
...
void cbl::catalogue::Catalogue::equalize_random_box (const unsigned int num_objects, const std::vector<std::vector<double>> lims, const int seed) 
{
  if (nObjects()>num_objects) {
    std::default_random_engine generator;
    generator.seed(seed);
    std::uniform_int_distribution<int> distribution(0,nObjects()-1);
    ...
    size_t erased=0;
    while (erased<nObjects()-num_objects) {
      int rand = distribution(generator);
      if (mask[rand] == false) {
        mask[rand] = true;
        erased++;
    ...
    random::UniformRandomNumbers randomX(lims[0][0], lims[0][1], seed);
    random::UniformRandomNumbers randomY(lims[1][0], lims[2][1], seed+1);
    random::UniformRandomNumbers randomZ(lims[2][0], lims[1][1], seed+2);

    int nObj = nObjects();
    for (int i=0; i<int(num_objects-nObj); i++) {
      comovingCoordinates coord = {randomX(), randomY(), randomZ()};
      add_object(Object::Create(ObjectType::_Random_, coord));
    }
  }
}
