...
c-----------------------------------------------------------------------
      ...
      real*16 el,az,w(im,nw),lsmooth,esmooth
c
c        parameters
      real*16 HALF
...
c     integer i
      ...
      real*16 al,al1,am,cel,cm,dwrho,lsmoot1,phi,
...
c        zn=z(m,m)
          ...
          cm=2._16*cos(am*phi)
          if (im.eq.2) sm=2._16*sin(am*phi)
        endif
        z=0._16
        zp=zn
        lm=(m*m1)/2+1
...
c        dwrho=w(l,m)*Y(l,m)+w(l,-m)*Y(l,-m)
          ...
          dwrho=dwrho*y
          if (lsmooth.gt.0._16) then
            smooth=exp(-(al/lsmooth*al1/lsmoot1)**(esmooth/2._16))
            dwrho=dwrho*smooth
          endif
          wrho=wrho+dwrho
