...
''' Define the cosmology and the threepointcorrelation '''
cosmology = cbl.LCDM("Planck18")
threeptcorr = cblCosmology.ThreePointCorrelation(cosmology)
PX = cblCosmology.PkXi(cosmology)

''' Compute Pk '''
redshift = 1
kk = np.logspace(-4, 2, 200)
Pk_matter = np.array( PX.Pk_matter(kk, "CAMB", False, redshift) )
...
''' Slepian model '''
zeta_DM_s = np.array( threeptcorr.zeta_DM(r1, r2, theta, "Slepian", kk, Pk_matter) )
q_DM_s = np.array( threeptcorr.Q_DM(r1, r2, theta, "Slepian", kk, Pk_matter) )

''' Barriga-Gatzagnaga model '''
zeta_DM_bg = np.array( threeptcorr.zeta_DM(r1, r2, theta, "BarrigaGatzanaga", kk, Pk_matter) )
q_DM_bg = np.array( threeptcorr.Q_DM(r1, r2, theta, "BarrigaGatzanaga", kk, Pk_matter) )
...
''' NonLocal Term '''
q_DM_NL = np.array( threeptcorr.Q_nonLocal(r1, r2, theta, kk, Pk_matter) )
