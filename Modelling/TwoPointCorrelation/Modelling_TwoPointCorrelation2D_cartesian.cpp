...
void cbl::modelling::twopt::Modelling_TwoPointCorrelation2D_cartesian::set_fiducial_xiDM ()
{
  ...
  if (m_data_model->sigmaNL==0) {  
    for (size_t i=0; i<(size_t)m_data_model->step; i++)
      xiDM[i] = PX.xi_matter(rad[i], m_data_model->method_Pk, m_data_model->NL, m_data_model->redshift, true, m_data_model->output_root, m_data_model->norm, m_data_model->k_min, m_data_model->k_max, m_data_model->aa, m_data_model->GSL, m_data_model->prec, m_data_model->file_par);
    ...
    vector<double> Pk = PX.Pk_matter_DeWiggled("CAMB", "EisensteinHu", kk, m_data_model->redshift, m_data_model->sigmaNL, 4, 10, 0.05, m_data_model->store_output, m_data_model->output_root, m_data_model->norm, m_data_model->prec);
    xiDM = Xi0(rad, kk, Pk);
  }
