...
std::vector<double> cbl::modelling::numbercounts::size_function_model (const std::vector<double> radii, const std::shared_ptr<void> inputs, std::vector<double> &parameter)
{
  ...
  return modelling::numbercounts::size_function(cosmo, radii, pp->redshift, pp->model_SF, bias_eff, bias_slope, bias_offset, pp->deltav_NL, pp->delta_c, pp->method_Pk, pp->store_output, pp->output_root, pp->interpType, pp->k_max, pp->input_file, pp->is_parameter_file);
