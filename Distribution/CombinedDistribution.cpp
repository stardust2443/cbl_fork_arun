...
cbl::glob::CombinedDistribution::CombinedDistribution (const DistributionType distributionType, const std::vector<double> meanVec, const std::vector<std::vector<double>> covMat, const std::vector<double> xMinVec, const std::vector<double> xMaxVec, const int seed) 
{
  ...
  m_xMinVec = xMinVec;
  m_xMaxVec = xMaxVec;
  vector<double> sigmaVec = cbl::wrapper::eigen::EigenToVector(cbl::wrapper::eigen::MatrixToEigen(covMat).diagonal().array().sqrt());
  ...
  m_inputs = make_shared<glob::STR_multivariateGaussian>(parameters);  
  ...
  for (size_t i=0; i<meanVec.size(); i++)
    m_distributionVec[i] = make_shared<cbl::glob::Distribution>(cbl::glob::Distribution(cbl::glob::DistributionType::_Gaussian_, {meanVec[i], sigmaVec[i]}, xMinVec[i], xMaxVec[i], seed+i));
...
cbl::glob::CombinedDistribution::CombinedDistribution (const std::string filename, const std::string path, const std::vector<int> columns_to_read, const int skip_nlines, const int type_data, const bool normalize, const int distNum, const double rMAX, const double cell_size) 
{
  ...
  vector<vector<double>> all_data = cbl::read_file(filename, path, columns_to_read, skip_nlines);
  vector<double> posterior;

  switch (type_data) {
  case 0:
    coutCBL << "Reading log(Posterior) distribution as column number " << columns_to_read[nparams] << endl;
  ...
  case 1:
    coutCBL << "Reading Posterior distribution as column number " << columns_to_read[nparams] << endl;
  ...
  case 2:
    coutCBL << "Reading Chi2 distribution as column number " << columns_to_read[nparams] << endl;
    posterior = all_data[nparams];
  ...
  vector<vector<double>> data(nparams);
  ...
  m_inputs = make_shared<glob::STR_chainMeshInterpolate>(parameters);  
  ...
  for (int i=0; i<nparams; i++)
    m_distributionVec[i] = make_shared<cbl::glob::Distribution>(cbl::glob::Distribution(cbl::glob::DistributionType::_Uniform_, m_xMinVec[i], m_xMaxVec[i]));
...
double cbl::glob::CombinedDistribution::operator[] (vector<double> xx)
{ 
  double fact = m_Func(xx, m_inputs);
  
  if (fact==0.) fact = DBL_MIN;
  
  else if (!isfinite(fact)) {
    fact = DBL_MIN;
