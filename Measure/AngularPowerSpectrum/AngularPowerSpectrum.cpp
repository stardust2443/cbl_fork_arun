...
cbl::measure::angularpk::PowerSpectrum_angular::PowerSpectrum_angular (const catalogue::Catalogue data, const double l_min, const double l_max, const int bandwidth, const std::string mask_file, const std::string mask_type, const double pixel_area, const int n_lines_header)
{
  m_l_min = l_min;   
  m_l_max = l_max;
...
std::vector<double> cbl::measure::angularpk::PowerSpectrum_angular::m_error (const BinType binType, std::vector<double> w_err)
{
  ...
  for (size_t j=0; j<AngularPowerSpectrum_err.size(); j++) {    
    for (size_t i=0; i<w_err.size();i++) {   
      ...
      Gp = m_theta[i]*m_l[j]*sqrt(std::exp(dlogl)) * boost::math::cyl_bessel_j(1, m_theta[i]*m_l[j]*sqrt(std::exp(dlogl))) 
     - m_theta[i]*m_l[j]/sqrt(std::exp(dlogl)) * boost::math::cyl_bessel_j(1, m_theta[i]*m_l[j]/sqrt(std::exp(dlogl)));
      fp = m_w_err[i]*Gp;    
...
void cbl::measure::angularpk::PowerSpectrum_angular::measureFast (const string dir_correlation_input, const string file_correlation_input, const int n_lines_header, CoordinateUnits inputUnits, BinType input_binType, cbl::measure::ErrorType errorType, const string dir_correlation_output, const string file_correlation_output, const int nMocks)
{
  ...
  for (size_t j=0; j<AngularPowerSpectrum.size(); j++) {   
    for (size_t i=0; i<m_w.size();i++) {   
      ...
      pref = p0*d/(m_theta[i]*m_theta[i]);
      Gp = m_theta[i]*m_l[j]*sqrt(std::exp(dlogl))*boost::math::cyl_bessel_j(1,m_theta[i]*m_l[j]*sqrt(std::exp(dlogl)))-m_theta[i]*m_l[j]/sqrt(std::exp(dlogl))*boost::math::cyl_bessel_j(1,m_theta[i]*m_l[j]/sqrt(std::exp(dlogl)));
      fp = m_w[i] * Gp;
...
void cbl::measure::angularpk::PowerSpectrum_angular::measureSphericalArmonic ()
{
  ...
  m_Wl.erase(std::begin(m_Wl), std::end(m_Wl));
  m_AngularPowerSpectrum.erase(std::begin(m_AngularPowerSpectrum), std::end(m_AngularPowerSpectrum));

  const double pi_2 = cbl::par::pi*0.5;
  const double _pi_4 = 1./cbl::par::pi*0.25;
  const double survey_area = m_survey_area;
  const double sigma0 = m_catalogue->nObjects()/survey_area;
  const double fsky = survey_area*_pi_4;    //fraction of the sky covered by the survey
  const int l_min = m_l_min;
  const int l_max = m_l_max;
  const int size_l = l_max-l_min;
  std::vector<double> l_(size_l), Cl_(size_l), Wl_(size_l), AngularPowerSpectrum_err_(size_l);
  
  coutCBL << "fsky = " << fsky << endl << endl;
  
#pragma omp parallel num_threads(omp_get_max_threads()<size_l ? omp_get_max_threads() : size_l)
  { 
#pragma omp for schedule(dynamic, 2)    
    for (int l=l_min; l<l_max; l++) { //works only with l<=879
	...
	if (fsky>0.9) {    //total survey
	  ...
	  double I_lm_re = CW_real.IntegrateCuhre(integration_limits, false);
	  cbl::wrapper::cuba::CUBAwrapper CW_im(integrand_im, ndim);
	  double I_lm_im = CW_im.IntegrateCuhre(integration_limits, false);
	  std::complex<double> I_lm (I_lm_re, -1.*I_lm_im);//complex conjugate
	  cbl::wrapper::cuba::CUBAwrapper CW_J_lm(integrand_J_lm, ndim);
	  double J_lm = CW_J_lm.IntegrateCuhre(integration_limits, false);
      ...
      C_l = C_l/(2*l+1)/(sigma0*sigma0);
      l_[counter] = l;
      Cl_[counter] = C_l;
      Wl_[counter] = W_l/(2*l+1);
      AngularPowerSpectrum_err_[counter] = sqrt(2/fsky/(2*l+1))*(C_l+1/sigma0);
  ...
  for (int ii=0; ii<size_l; ii++)
      coutCBL << l_[ii] << "   " << Cl_[ii] << endl;
  
  m_l.insert(std::end(m_l), std::begin(l_), std::end(l_));
  m_Wl.insert(std::end(m_Wl), std::begin(Wl_), std::end(Wl_));
  m_AngularPowerSpectrum.insert(std::end(m_AngularPowerSpectrum), std::begin(Cl_), std::end(Cl_));
  m_AngularPowerSpectrum.insert(std::end(m_AngularPowerSpectrum), std::begin(AngularPowerSpectrum_err_), std::end(AngularPowerSpectrum_err_));
